...
class Skiplist(object):
    ...
    def add(self, num):
        ...
        :rtype: None
        """
        # Start at the lowest level and keep flipping coins to decide 
        # whether to insert an entry. 1 is yes and 0 is not.
        k = 1
        p = random.choice([0, 1])
        while (p == 1) and (k < len(self.S)):
            ...
            p = random.choice([0, 1])
        if k == len(self.S):
            # If an entry will be inserted at the highest level, 
            # expand the number of levels.
            nr = Node(None)
            nr.down = self.S[-1]
            self.S.append(nr)
        ...
        par = None
        while curr is not None:
            # Traverse to the right in the current level as far a
